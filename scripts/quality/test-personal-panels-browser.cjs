    ...
    async function setup({ query = '', permissions = ['dashboard.view', 'tickets.view', 'schedule.view', 'inventory.view'], role = 'Gestor', views = all, savedViews = views, active = false, mirror = false, mobile = false, loggedOut = false, fail = {}, empty = false, malformed = false, legacy = false, former = false, viewport, dense = false } = {}) {
        ...
        await context.route('**/*', async route => {
            ...
            const data = {
                ...
                'atendimentos-clientes': { generatedAt, period, source: 'CLIENT', scope: 'ALL', openedInPeriod: 1, closedInPeriod: 0, current: { pending: 1, inProgress: 0, waitingClient: 0, resolved: 0, needingAttention: 1, averagePendingSeconds: 5400, oldestPendingAt: clientTicket.createdAt }, sectors: [{ source: 'CLIENT', name: 'Cliente QA', openedInPeriod: 1 }] },
            ...
            if (state.dense) {
                ...
                data['atendimentos-clientes'].sectors = [{ source: 'CLIENT', name: 'Cliente QA', openedInPeriod: 8 }];
