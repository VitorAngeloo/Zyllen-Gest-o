...
(async () => {
    ...
    for (const [name, sql] of Object.entries(queries)) {
        const relevant = x => (tables.includes(x.table_name) || ['Location','Schedule'].includes(x.table_name)) && x.indexname !== 'VehicleUse_one_active_per_vehicle';
        const left = (await actual.query(sql)).rows.filter(relevant);
        const right = (await expected.query(sql)).rows.filter(relevant);
        assert.deepEqual(left, right, 'Migration chain differs from Prisma schema: ' + name);
    }
    assert.equal((await actual.query(`SELECT COUNT(*)::int AS count FROM pg_indexes WHERE schemaname='public' AND indexname='VehicleUse_one_active_per_vehicle'`)).rows[0].count, 1);
    ...
    console.log(`PASS all ${tables.length} new tables retain private RLS`);
