...
export class InventoryController {
    ...
    async createEntry(@Request() req: any, @Body() body: {
        skuId: string;
        toLocationId: string;
        qty: number;
    ...
    async createExit(@Request() req: any, @Body() body: {
        skuId: string;
    ...
    async approveExit(@Param('id') id: string, @Request() req: any, @Body() body: { pin: string }) {
    ...
    @HttpCode(HttpStatus.OK)
    async rejectExit(@Param('id') id: string, @Request() req: any, @Body() body: { pin: string }) {
    ...
    async requestReversal(@Param('id') id: string, @Request() req: any, @Body() body: { reason: string }) {
    ...
    @RequirePermission('approvals.approve')
    @HttpCode(HttpStatus.OK)
    async approveReversal(@Param('id') id: string, @Request() req: any, @Body() body: { pin: string }) {
    ...
    async findAllMovements(
        ...
        const data = await this.inventoryService.findAllMovements({ skuId, locationId, typeId });
        return { data };
    }
    ...
    async createMovementType(@Body() body: { name: string; requiresApproval?: boolean; isFinalWriteOff?: boolean; setsAssetStatus?: string }) {
