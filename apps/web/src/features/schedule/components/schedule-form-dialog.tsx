"use client";
import { SCHEDULE_TYPES, SCHEDULE_TYPE_LABELS, type ScheduleType } from '@zyllen/shared';
import type { AgendaInstaller as Installer, FormSchedule as Schedule, Company, Project, ConflictRow } from '../types/schedule.types';
import { toDatetimeLocal, formatDate as formatDatetime } from '../utils/schedule-format';
import { DEFAULT_COLOR, inputCls } from '../schedule.constants';
...
export function ScheduleFormDialog({
    ...
    const { data: installersRes, isLoading: loadingInstallers } = useQuery({
        ...
        queryFn: () => scheduleApi.listActiveInstallers<{ data: Installer[] }>(fetchOpts),
        enabled: open,
    });

    const { data: companiesRes } = useQuery({
        queryKey: ["companies-list"],
        queryFn: () => scheduleApi.listCompanies<{ data: Company[] }>(fetchOpts),
        enabled: open,
    });
    ...
    const { data: projectsRes } = useQuery({
        ...
        queryFn: () =>
            scheduleApi.listProjects<{ data: Project[] }>(form.companyId, fetchOpts),
    ...
    if (editingSchedule?.projectService) for (const user of editingSchedule.installers) {
        if (!installers.some(item => item.id === user.id)) installers.push({ ...user, sector: null, agendaActive: true });
    }
    ...
    const updateMut = useMutation({
        mutationFn: (data: typeof form) =>
            scheduleApi.updateSchedule(editingSchedule!.id, {
                ...data,
                allowConflicts: step === 'conflicts',
                startDate: new Date(data.startDate).toISOString(),
                endDate: new Date(data.endDate).toISOString(),
    ...
    function validate(): string | null {
        ...
        if (form.installerIds.length === 0 && !editingSchedule?.projectService?.contractors.length) return "Selecione pelo menos 1 técnico";
    ...
    const handleSubmit = useCallback(async () => {
        ...
        try {
            ...
            const res = await scheduleApi.listConflicts<{ data: ConflictRow[] }>(params, fetchOpts);
                ...
                {step === "form" && (
                    <>
                        <DialogBody className="space-y-5">
                            {editingSchedule?.projectService && <div className="rounded-lg border border-[var(--zyllen-border)] p-3 text-xs text-[var(--zyllen-muted)]"><p>{PROJECT_SERVICE_COPY.linkedAgenda}</p><a href="/dashboard/projetos" className="mt-2 block text-[var(--zyllen-highlight)]">{PROJECT_SERVICE_COPY.manageProject}</a>{editingSchedule.projectService.contractors.map(({ user }) => <p key={user.id} className="mt-1">{user.name} · {PROJECT_SERVICE_COPY.contractor}</p>)}</div>}
                            {/* Title + Type */}
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    >
                                        {SCHEDULE_TYPES.filter(t => !editingSchedule?.projectService || t === 'INSTALLATION' || t === 'REMOVAL').map((t: ScheduleType) => (
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div className="space-y-1.5">
                                    ...
                                    <select
                                        ...
                                        disabled={!!editingSchedule?.projectService}
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <select
                                        ...
                                        disabled={!!editingSchedule?.projectService || !form.companyId || projects.length === 0}
                                    >
                                        <option value="">
