...
function MetricCard({ metric, featured = false }: { metric: { key: string; label: string; value: number; context: string }; featured?: boolean }) {
    return <div data-operation-metric={metric.key} className={`min-w-0 rounded-md border p-4 ${metricTones[metric.key] ?? 'border-white/10 bg-white/[0.025]'} ${featured ? 'sm:flex sm:flex-col sm:justify-between' : ''}`}>
...
function ServiceSection({ title, context, items, history = false }: { title: string; context: string; items: OperationServiceHighlight[]; history?: boolean }) {
    return <section aria-label={title} className="min-w-0 overflow-hidden rounded-md border border-white/10 bg-black/10">
...
function TripSection({ data }: { data: OperationsStatistics['nextInterstateTrips'] }) {
    return <section aria-label={copy.nextTrips} className="min-w-0 overflow-hidden rounded-md border border-white/10 bg-black/10">
...
export function OperationsDashboardView({ data, loading, failed, fetching, onRetry }: Props) {
    ...
    return <div className="space-y-5">
        {failed && <div role="alert" className="rounded-md border border-red-400/50 bg-red-500/10 p-4 text-sm text-red-200"><p>{copy.loadError}</p>{data && <p className="mt-1">{copy.stale}</p>}<Button type="button" variant="ghost" size="sm" className="mt-2" disabled={fetching} onClick={onRetry}>{copy.retry}</Button></div>}

        {loading ? <div className="grid gap-4 xl:grid-cols-[minmax(17rem,0.55fr)_minmax(0,1.45fr)]">
            <div className="rounded-lg border border-white/10 bg-black/10 p-4"><Skeleton className="h-4 w-32" /><div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-1">{Array.from({ length: 2 }, (_, index) => <Skeleton key={index} className="h-28" />)}</div></div>
            <div className="rounded-lg border border-white/10 bg-black/10 p-4"><Skeleton className="h-4 w-36" /><div className="mt-4 grid gap-3 sm:grid-cols-2">{Array.from({ length: 4 }, (_, index) => <Skeleton key={index} className="h-28" />)}</div></div>
        </div> : data && <>
