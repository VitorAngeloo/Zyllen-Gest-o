...
import {
    ...
    RefreshCw, PenLine, Check, ChevronRight, Loader2, Lock,
} from "lucide-react";
...
import { OSFollowupSection, InlineSignaturePad } from "@web/features/maintenance/components/os-forms/os-followup-section";
...
export default function ClientMaintenancePage() {
    ...
    const { data, isLoading, refetch } = useQuery({
        ...
        queryFn: () => maintenanceApi.listClientOrders<{ data: any[]; total: number }>(fetchOpts),
    });
    ...
    const signWitnessMutation = useMutation({
        mutationFn: (sig: string) =>
            maintenanceApi.confirmWitnessSignature(selectedOS?.id, { signature: sig }, fetchOpts),
        onSuccess: () => {
    ...
    return (
        <div className="space-y-6">
            <PageHeader
                ...
                description="Instalações de sala vinculadas à sua empresa"
                actions={<button
                    type="button"
                    onClick={() => refetch()}
            ...
            <ListSectionHeader
                ...
                count={osList.length}
                description="Consulte os dados, o formulário e as confirmações de cada ordem."
            />
            ...
            {isLoading ? (
                ...
                <EmptyState
                    ...
                    description="As ordens vinculadas à sua empresa aparecerão aqui."
                />
            ) : (
                <RecordList>
                    {osList.map((os) => {
                            ...
                            >
                                <div className="flex items-center justify-between gap-4">
                                    <div className="flex min-w-0 items-center gap-3">
                                        <Wrench size={18} className="shrink-0 text-[var(--zyllen-highlight)]" />
