...
export default function EquipamentosPage() {
    ...
    return (
        <div className="space-y-6">
            <PageHeader eyebrow="Estoque e patrimônio" title="Equipamentos" description={PAGE_DESCRIPTIONS.equipamentos} />

            {/* Tabs */}
            <div role="tablist" aria-label="Áreas de equipamentos" className="flex max-w-full flex-wrap gap-1 border-b border-white/10">
                {tabs.map((t) => (
                    <button
                        key={t.key}
            ...
            {tab === "estoque" && (
                <div className="space-y-6">
                    ...
                    <WorkspaceBar className="sm:block">
                        <WorkspaceGroup label="Consultar equipamentos">
                            <div className="relative max-w-xl">
                                <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--zyllen-muted)]" />
                                <Input
                                    value={search}
                                    onChange={(e) => setSearch(e.target.value)}
                                    placeholder="Nome, código do item, descrição ou código de barras..."
                                    className="pl-9"
                                />
                            </div>
                        </WorkspaceGroup>
                    ...
                    <ListSectionHeader
                        ...
                        count={loadingEquipment ? undefined : (equipment?.total ?? 0)}
                        description={search ? "Resultados da busca atual e distribuição disponível por local." : "Itens, patrimônios e quantidades disponíveis no estoque."}
                    />
                    <div>
                            {loadingEquipment ? (
                                <div className="divide-y divide-white/10 border-y border-white/10">
                                    {[...Array(5)].map((_, i) => (
                                        <Skeleton key={i} className="h-14 w-full rounded-none bg-white/[0.025]" />
                                    ))}
                                </div>
                                    ...
                                    <table className="w-full min-w-[900px] text-sm">
                                        <thead>
                                            <tr className="border-b border-white/10 bg-white/[0.015]">
                                                {['Código', 'Nome', 'Descrição', 'Categoria', 'Marca', 'Patrimônios', 'Estoque', 'Locais', 'Ações'].map((heading, index) => (
                                                    <th key={heading} scope="col" className={`px-3 py-3 text-[10px] font-semibold uppercase tracking-[0.12em] text-white/40 ${index >= 5 && index <= 7 ? 'text-center' : index === 8 ? 'text-right' : 'text-left'} ${index === 2 ? 'hidden md:table-cell' : ''} ${index === 3 || index === 4 ? 'hidden lg:table-cell' : ''}`}>{heading}</th>
                                                ))}
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-white/10">
                                            {equipment.data.map((item) => (
                                                <Fragment key={item.id}>
                                                    <tr
                                                        tabIndex={0}
                                                        aria-label={`${expandedSku === item.id ? "Recolher" : "Expandir"} distribuição de ${item.name}`}
                                                        ...
                                                        onKeyDown={(event) => { if (event.target === event.currentTarget && (event.key === "Enter" || event.key === " ")) { event.preventDefault(); setExpandedSku(expandedSku === item.id ? null : item.id); } }}
                                                    >
                                                        <td className="px-3 py-3 font-mono text-xs text-white/70">{item.skuCode}</td>
                                                        <td className="px-3 py-3 font-medium text-white">{item.name}</td>
                                                        <td className="hidden max-w-[200px] truncate px-3 py-3 text-xs text-[var(--zyllen-muted)] md:table-cell">{item.description || "—"}</td>
                                                        <td className="hidden px-3 py-3 lg:table-cell">
                                                            <Badge variant="secondary">{item.category?.name}</Badge>
                                                        </td>
                                                        <td className="hidden px-3 py-3 text-[var(--zyllen-muted)] lg:table-cell">{item.brand || "—"}</td>
                                ...
                                <EmptyState icon={<Package size={24} />} title={search ? "Nenhum equipamento encontrado" : "Nenhum equipamento cadastrado"} description={search ? "Revise a busca para localizar outros itens." : EMPTY_STATES.equipment} />
                            )}
                    </div>
                </div>
            )}
