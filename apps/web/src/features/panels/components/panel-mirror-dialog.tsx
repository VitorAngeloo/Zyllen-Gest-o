...
export function PanelMirrorDialog({ views, onClose }: { views: PanelId[]; onClose: () => void }) {
    ...
    const status = useQuery({ queryKey: ['panel-mirror-status', user?.id], queryFn: () => panelApi.status(options) });
    ...
    return <Dialog open onOpenChange={open => { if (!open && !busy) onClose(); }}><DialogContent ref={ref} role="dialog" tabIndex={-1} aria-modal="true" aria-labelledby={titleId}>
        ...
        </DialogBody><DialogFooter className="flex-wrap"><Button variant="outline" disabled={busy} onClick={onClose}>{copy.close}</Button>{status.data?.active && <Button variant="outline" disabled={busy} onClick={() => revoke.mutate()}>{copy.revoke}</Button>}<Button disabled={busy || !selected.length || !status.data || status.isError} onClick={() => generate.mutate()}>{busy ? copy.saving : status.data?.active ? copy.regenerate : copy.generate}</Button></DialogFooter>
