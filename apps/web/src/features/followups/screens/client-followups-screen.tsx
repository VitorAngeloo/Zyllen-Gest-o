...
import { ClipboardList, ArrowLeft, ChevronRight, RefreshCw, FileText, Image as ImageIcon, CheckSquare, Upload, Video } from "lucide-react";
...
export default function ClientFollowupsPage() {
    ...
    const { data, isLoading, refetch } = useQuery({
        ...
        queryFn: () => followupsApi.listClientFollowups<{ data: any[]; total: number }>(fetchOpts),
    });

    // Fetch full detail (with blocks) only when a followup is selected
    const { data: detailData, isLoading: detailLoading } = useQuery({
        queryKey: ["client-followup-detail", selectedId],
        queryFn: () => followupsApi.getClientFollowup<{ data: any }>(selectedId, fetchOpts),
        enabled: !!selectedId,
    });
    ...
    return (
        <div className="space-y-6">
            <PageHeader
                ...
                description="Acompanhamentos vinculados à sua empresa"
                actions={<button
                    type="button"
                    onClick={() => refetch()}
            ...
            <ListSectionHeader
                title="Registros compartilhados"
                count={list.length}
                description="Abra um acompanhamento para consultar blocos, arquivos e atualizações."
            />
            ...
            {isLoading ? (
                ...
                <EmptyState
                    ...
                    description="Os acompanhamentos compartilhados com sua empresa aparecerão aqui."
                />
            ) : (
                <RecordList>
                    {list.map((followup) => {
                            ...
                            >
                                <div className="flex items-center justify-between gap-4">
                                    <div className="flex min-w-0 items-center gap-3">
                                        <ClipboardList size={18} className="shrink-0 text-[var(--zyllen-highlight)]" />
                                    ...
                                    <div className="flex shrink-0 items-center gap-3">
                                        <Badge variant={status.variant}>{status.label}</Badge>
                                        <ChevronRight size={16} className="text-white/25 transition-transform group-hover:translate-x-0.5 group-hover:text-[var(--zyllen-highlight)]" />
                                    </div>
                                </div>
                            </RecordRow>
                        );
