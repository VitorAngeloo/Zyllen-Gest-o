...
import {
    ...
    CheckCircle2, AlertCircle, Save, X, Trash2, Power,
} from "lucide-react";
    ...
    return (
        <div className="space-y-6">
            ...
            <PageHeader
                ...
                title={user.name}
                description="Dados de acesso, vínculo com a operação e histórico recente desta pessoa."
                actions={canManage && !editing && (
                    <div className="flex flex-wrap gap-2">
                        <Button variant="outline" size="sm" onClick={startEdit}><Pencil size={14} className="mr-1" /> Editar</Button>
                        <Button variant="outline" size="sm" className={user.isActive ? "border-red-500/30 text-red-400 hover:bg-red-500/10" : "border-green-500/30 text-green-400 hover:bg-green-500/10"} onClick={() => { if (confirm(`${user.isActive ? "Desativar" : "Ativar"} ${user.name}?`)) toggleActive.mutate(!user.isActive); }}><Power size={14} className="mr-1" /> {user.isActive ? "Desativar" : "Ativar"}</Button>
                        <Button variant="outline" size="sm" className="border-red-500/30 text-red-400 hover:bg-red-500/10 hover:text-red-300" onClick={handleDelete} disabled={deleteUser.isPending}><Trash2 size={14} className="mr-1" /> Excluir</Button>
                    </div>
                )}
            />
            ...
            {/* Tabs */}
            <div role="tablist" aria-label="Áreas do colaborador" className="flex max-w-full gap-1 overflow-x-auto border-b border-white/10">
                {TABS.map((tab) => (
                    <button
                        key={tab.key}
                    ...
                    {editing ? (
                        ...
                        <Card className="rounded-none border-x-0 border-white/10 bg-transparent">
                            <CardContent className="px-0 py-0">
                                <div className="grid gap-5 border-y border-white/10 py-5 md:grid-cols-2 md:px-4">
                                    <InfoRow label="Nome" value={user.name} />
                                    <InfoRow label="Email" value={user.email} />
                                    <InfoRow label="Perfil de acesso" value={user.role.name} />
