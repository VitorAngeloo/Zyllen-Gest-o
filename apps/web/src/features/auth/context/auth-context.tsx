...
export function AuthProvider({ children, anonymous = false }: { children: React.ReactNode; anonymous?: boolean }) {
    ...
    useEffect(() => {
        if (anonymous) { setIsLoading(false); return; }
        setIsLoading(true);
    ...
    return (
        <AuthContext.Provider value={anonymous ? { user: null, token: null, permissions: [], userType: null, isLoading: false, needsPin: false,
            login: async () => { throw new Error('Este espelho permite somente leitura'); }, logout: () => {}, hasPermission: () => false, clearNeedsPin: () => {} }
            : { user, token, permissions, userType, isLoading, needsPin, login, logout, hasPermission, clearNeedsPin }}>
            {children}
