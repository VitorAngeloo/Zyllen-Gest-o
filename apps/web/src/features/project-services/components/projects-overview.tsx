...
export function ProjectsOverview() {
    ...
    return <div className="space-y-6">
        ...
        {period && <>
            <section aria-label={copy.overviewProjects} className="space-y-5 rounded-2xl border border-white/10 bg-white/[0.018] p-4 shadow-[0_18px_60px_rgba(0,0,0,0.16)] sm:p-5"><ListSectionHeader title={copy.overviewProjects} description={projectCopy.scope} actions={<label className="min-w-52 text-xs text-[var(--zyllen-muted)]">{projectCopy.type}<Select aria-label={projectCopy.type} className="mt-1" value={type} onValueChange={value => setType(value as ProjectStatisticsQuery['type'])}><SelectOption value="ALL">{projectCopy.allTypes}</SelectOption>{Object.entries(serviceCopy.types).map(([value, label]) => <SelectOption key={value} value={value}>{label}</SelectOption>)}</Select></label>} />
            ...
            <section aria-label={copy.overviewTrips} className="space-y-5 rounded-2xl border border-white/10 bg-white/[0.018] p-4 shadow-[0_18px_60px_rgba(0,0,0,0.16)] sm:p-5"><ListSectionHeader title={copy.overviewTrips} description={operationsCopy.scope} />
