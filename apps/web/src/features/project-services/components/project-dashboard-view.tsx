...
function MetricCard({ metric, featured = false }: { metric: { key: string; label: string; value: number; context: string }; featured?: boolean }) {
    return <div data-project-metric={metric.key} className={`min-w-0 rounded-xl border p-4 ${metricTones[metric.key] ?? 'border-white/10 bg-white/[0.025]'} ${featured ? 'sm:row-span-2 sm:flex sm:flex-col sm:justify-between' : ''} ${metric.key === 'scheduled' ? 'xl:col-span-2' : ''}`}>
...
export function ProjectDashboardView({ data, loading, failed, fetching, onRetry }: Props) {
    ...
    return <div className="space-y-5">
        ...
        {loading ? <div className="grid gap-4 xl:grid-cols-[minmax(0,1.45fr)_minmax(17rem,0.55fr)]">
            <div className="rounded-2xl border border-white/10 bg-black/10 p-4"><Skeleton className="h-4 w-32" /><div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{Array.from({ length: 4 }, (_, index) => <Skeleton key={index} className={`h-28 ${index === 0 ? 'sm:row-span-2 sm:h-auto' : ''}`} />)}</div></div>
            <div className="rounded-2xl border border-white/10 bg-black/10 p-4"><Skeleton className="h-4 w-36" /><div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-1">{Array.from({ length: 2 }, (_, index) => <Skeleton key={index} className="h-28" />)}</div></div>
        </div> : data && <>
            ...
            {(missingDates || data.dataQuality.unclassified > 0) && <div role="status" className="rounded-xl border border-amber-400/25 bg-amber-400/[0.055] px-4 py-3 text-sm text-amber-100"><p className="font-medium">{copy.qualityTitle}</p>{missingDates && <p className="mt-1 text-amber-100/75">{copy.missingDates(data.dataQuality.completedWithoutDate, data.dataQuality.cancelledWithoutDate)}</p>}{data.dataQuality.unclassified > 0 && <p className="mt-1 text-amber-100/75">{copy.unclassified(data.dataQuality.unclassified)}</p>}</div>}
            ...
            <section aria-label={copy.highlights} className="space-y-3">
                ...
                {data.highlights.length ? <ul className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{data.highlights.map(project => {
                    ...
                    return <li key={project.id} data-project-highlight={project.id} className="relative min-w-0 overflow-hidden rounded-xl border border-white/10 px-4 py-4 transition-colors hover:border-white/20" style={{ backgroundImage: `linear-gradient(135deg, ${color}1f, rgba(255,255,255,0.018) 58%)`, boxShadow: `inset 3px 0 0 ${color}` }}>
