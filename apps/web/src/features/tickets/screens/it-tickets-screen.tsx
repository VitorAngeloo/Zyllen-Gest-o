    ...
    return (
        <div className="space-y-6">
            <PageHeader
                ...
                title="Meus Chamados TI"
                description="Abra uma solicitação, acompanhe quem está atendendo e continue a conversa no mesmo registro."
                actions={<Button variant="highlight" onClick={() => setShowNewForm(true)}>
            ...
            <ListSectionHeader
                ...
                description="Os registros mais recentes ficam disponíveis para consulta e continuidade do atendimento."
            />

            {loading ? (
                <div className="divide-y divide-white/10 border-y border-white/10">
                    {[...Array(3)].map((_, i) => (
                        <div key={i} className="h-20 bg-white/[0.025] animate-pulse" />
                    ))}
                </div>
            ) : tickets.length === 0 ? (
                <EmptyState
                    icon={<Headset size={24} />}
                    title="Nenhum chamado aberto por você"
                    description="Quando precisar de suporte interno, descreva o pedido e anexe imagens ou vídeos que ajudem no diagnóstico."
                    action={
                        <Button variant="highlight" size="sm" onClick={() => setShowNewForm(true)}>
                        ...
                        return (
                            ...
                            >
                                <div className="flex items-center gap-4">
                                    <StatusIcon size={17} className="hidden shrink-0 sm:block" style={{ color: statusCfg.color }} aria-hidden="true" />
                                    <div className="min-w-0 flex-1">
                                        <p className="truncate text-sm font-semibold text-white group-hover:text-[var(--zyllen-highlight)]">{ticket.title}</p>
                                        <div className="mt-1 flex flex-wrap gap-x-3 gap-y-0.5 text-xs text-[var(--zyllen-muted)]">
                                            <span className="tabular-nums">Aberto em {new Date(ticket.createdAt).toLocaleDateString("pt-BR")}</span>
                                            <span>{ticket.assignedTo ? `Atendente: ${ticket.assignedTo.name}` : "Aguardando atendente"}</span>
                                        </div>
                                    </div>
                                    <div className="hidden shrink-0 items-center gap-4 md:flex">
                                        <span className="border-l border-white/15 pl-2 text-xs font-medium" style={{ color: priorityCfg.color }}>
                                            {priorityCfg.label}
                                        </span>
                                        <span className="border-l border-white/15 pl-2 text-xs font-medium" style={{ color: statusCfg.color }}>
                                            {statusCfg.label}
                                        </span>
                                    </div>
                                    {ticket.messages && ticket.messages.length > 0 && (
                                        <span className="flex shrink-0 items-center gap-1 font-mono text-[11px] text-[var(--zyllen-muted)]" aria-label={`${ticket.messages.length} mensagens`}>
                                ...
                                <div className="mt-2 flex items-center gap-3 md:hidden">
                                    <span className="border-l border-white/15 pl-2 text-[11px] font-medium" style={{ color: priorityCfg.color }}>{priorityCfg.label}</span>
                                    <span className="border-l border-white/15 pl-2 text-[11px] font-medium" style={{ color: statusCfg.color }}>{statusCfg.label}</span>
                                </div>
                            </RecordRow>
                        );
                    })}
