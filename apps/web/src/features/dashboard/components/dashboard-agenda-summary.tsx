...
export function DashboardAgendaSummary() {
    ...
    return <DashboardSummaryCard id="agenda" title={copy.agenda} icon={CalendarDays} href="/dashboard/projetos?aba=agenda&visao=calendario" linkLabel={copy.openAgenda}
        ...
        {query.data && <><p className="mb-3 text-xs text-[var(--zyllen-muted)]">{copy.agendaDescription}</p>{query.data.data.length ? <ul className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">{query.data.data.map(item => <li key={item.id}><button type="button" data-upcoming-schedule={item.id} onClick={() => setSelected(item)}
            className="h-full w-full rounded-lg border border-[var(--zyllen-border)] bg-[var(--zyllen-bg-dark)] p-3 text-left hover:border-[var(--zyllen-highlight)]/40">
