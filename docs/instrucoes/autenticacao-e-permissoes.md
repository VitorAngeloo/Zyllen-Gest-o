...
No catálogo, `Técnico` recebe `catalog.create_sku` para cadastrar itens (SKU) antes de registrar a entrada no estoque. `POST /catalog/skus` aceita `catalog.create_sku` ou o legado `catalog.create`; o cadastro de categorias continua exigindo `catalog.create`. Isso não concede edição/exclusão de itens nem gestão de categorias. A migration `20260921120000_technician_catalog_item_create` atribui a permissão de forma aditiva aos perfis Técnico existentes; a aplicação na base compartilhada segue a revisão de pendências e o procedimento de publicação, sem rodar o seed.

## Telas de permissão previstas no seed

`dashboard` · `schedule` (view, create, update, delete, manage_installers) · `vehicles` (view, reserve) · `inventory` (view, bipar_entrada, bipar_saida, historico, exit) · `assets` (view, create, lookup) · `catalog` (view, create, create_sku, update, delete) · `locations` · `suppliers` (view, create, update, delete) · `purchases` (view, create, approve, receive) · `tickets` (view, triage, assign, close) · `maintenance` (view, open, execute, close) · `approvals` (view, approve, reject) · `access` (view, manage, manage_roles, manage_permissions) · `labels` (view, print) · `followups` (view, create, edit, delete) · `audit` (view) · `settings` (view, manage)
