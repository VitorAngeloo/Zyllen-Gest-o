...
Correção da seleção de clientes de atenção em 23/09/2026: o backup privado `C:\Users\SERVIDOR ZYLLEN\Documents\Zyllen-Backups\attention-clients-20260923-231320` foi validado por catálogo e restauração completa em PostgreSQL isolado, com 62 tabelas e 19 migrations aplicadas. A única pendência, `20260923120000_panel_attention_clients`, foi revisada e aplicada com `prisma migrate deploy`; as 20 migrations ficaram em dia. A tabela nova foi confirmada vazia, com chave primária composta, duas FKs restritivas e RLS. A API já continha os endpoints compilados, permaneceu no PID **2656** sem reinício e respondeu `200` nos health checks local e público. Não houve seed, reset, criação de registros de teste nem alteração de dados existentes.
