...
O módulo `structures` mantém a identidade `OperationalStructure` e o histórico `StructureCycle`. Exporta `StructureCyclesService` para os módulos de projetos/agenda, concentrando validação e bloqueio transacional sem criar dependência circular. `features/structures` reúne seleção/cadastro inline de sala ou totem e consulta **Histórico de instalações**, incorporados à aba Projetos e ao detalhe do projeto. `StructureHistory` recebe callback opcional para abrir projeto na consulta de locais; não importa seu formulário. Dentro do detalhe, exibe história sem outro popup. `/dashboard/projetos/estruturas` é compatibilidade para a área unificada. Os contratos públicos ficam em `shared/src/structures`. Ver [estruturas e ciclos](estruturas-e-ciclos.md).
