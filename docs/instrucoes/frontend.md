...
- [UX design](ux-design.md): base de consulta para princípios, estados, formulários, navegação, acessibilidade e avaliação por tarefas. As próximas melhorias estão no [ciclo de UX de 20/09/2026](plano-identidade-visual.md#ciclo-de-ux-de-20092026); propostas desse ciclo ainda não descrevem comportamento implementado.
...
Contas internas com papel **Internos** recebem uma dashboard própria, somente para consulta. Ela mostra chamados internos `OPEN` e `IN_PROGRESS` abertos pela própria conta (`internalUserId`), o resumo e os destaques dos projetos atuais e a situação e as próximas reservas dos carros. Seus cartões não possuem links, popups ou ações de negócio. A sidebar desse papel contém **Dashboard**, **Meus Chamados TI**, **Acompanhamento** e **Carros**; o perfil continua acessível pelo controle da conta. Em Carros, Internos podem reservar, retirar e devolver, mas não cadastrar veículos, editar/cancelar reservas ou acessar o painel exclusivo de Administrador/Gestor. Acesso direto a outra rota interna da dashboard volta para `/dashboard`.
...
No registro da R2 de 18/09/2026, a API/migration de carros já estavam publicadas e o frontend ainda era apenas local. A publicação atual está registrada em [implantação](implantacao.md). A R3 reúne projetos/agenda/viagens em uma entrada, conforme [plano operacional](plano-evolucao-operacional-e-paineis.md); os links dos resumos abrem a aba correspondente. Verificações: `pnpm test:panels:browser`, após build isolado. Capturas/resultados em `tmp/architecture-validation/personal-panels-browser-qa/`; testes usam respostas sintéticas de loopback.
