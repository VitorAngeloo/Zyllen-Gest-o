...
**Atualização de 22/09/2026:** saídas individuais e em lote para clientes recebem o cliente e o projeto como destino. Na mesma transação da primeira saída, a API valida que o projeto pertence ao cliente, reutiliza seu estoque `CLIENT` se já existir ou cria e audita esse estoque antes de movimentar os patrimônios. Saídas posteriores reutilizam o mesmo local. Uma falha na saída também desfaz a criação do estoque, sem deixar um cadastro vazio parcial. Saídas por **Manutenção**, **Uso interno** e **Baixa** continuam selecionando automaticamente os locais internos correspondentes, sem exigir cliente/projeto. **Perda** e **Uso** não são opções de motivo de saída. A saída individual exige **Evento na timeline**. Baixa definitiva aguarda aprovação de outra pessoa; o patrimônio permanece consultável em **Baixa**, com condição `BAIXADO` e timeline. Saídas antigas sem destino específico passam por **Outros**, preservando local, movimento e timeline. A migração aditiva [20260921140000_internal_stock_destinations](../../apps/api/prisma/migrations/20260921140000_internal_stock_destinations/migration.sql) já foi aplicada à base compartilhada.

Em **Estoque → Patrimônios**, selecionar **Clientes** e a sala/projeto no filtro **Local** habilita **Desinstalar sala**. A conferência apresenta cada patrimônio e permite marcar os que retornaram, com contagem por produto. A conferência aguarda aprovação; até lá, nenhum item fica disponível. Depois da aprovação, os marcados voltam ao almoxarifado escolhido como `ATIVO` com evento **Voltou para estoque**; os não marcados vão para **Baixa** como `BAIXADO` com evento **Perda**, mantendo histórico e origem. Alterações simultâneas na sala invalidam a conferência antes de qualquer movimento. Em Entrada e Entrada em Lote, **Baixa**, **Manutenção** e **Outros** não aparecem como locais de entrada comuns; os campos de texto de entrada e saída desativam sugestões de preenchimento automático do navegador.
...
Na saída para cliente, selecionar a origem, o cliente e **um projeto pertencente a esse cliente**, além dos patrimônios. O destino mantém esses vínculos para permitir consultar onde cada unidade está e em qual projeto é usada. Se o projeto ainda não possuir estoque, a primeira saída o cria automaticamente e de forma atômica; as próximas localizam e reutilizam esse estoque. Envios e devoluções continuam preservando os registros de movimentação e auditoria. Um estoque por cliente não reúne projetos diferentes sem identificação do destino individual.

`Location` permite vincular cliente e projeto. O formulário de saída exige essa escolha e a API resolve o local específico sem depender de cadastro manual prévio. Preservar os locais e movimentos existentes; o local legado genérico `Cliente` e os patrimônios sem localização não recebem vínculos por suposição.
...
Esse local representa o estoque geral do cliente. Ele não elimina a separação por projeto: a primeira saída de patrimônio para um projeto cria seu local `CLIENT` específico, e as seguintes o reutilizam. A criação do estoque geral não executa backfill nem cria locais de projeto antecipadamente.
